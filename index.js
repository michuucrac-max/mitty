/**
 * index.js - Softti Tales (Definitivo con cola de música)
 * Funcionalidad completa: Música, IA, AutoMod, imágenes y comandos
 */
...
function getOrCreateGuildPlayer(guild){
  ...
  player.on(AudioPlayerStatus.Idle, async () => {
    if(entry.queue.length > 0){
      const nextSong = entry.queue.shift();
      await playSong(entry, nextSong);
    } else {
      entry.isPlaying = false;
      entry.currentSong = null;
    }
  });
...
async function playSong(g, songInfo){
  try{
    ...
    const textChannel = getTextChannel(g.guild);
    const voiceChannel = getVoiceChannel(g.guild);
    if(textChannel && voiceChannel){
      await textChannel.send(`🎵 Ahora reproduciendo **${songInfo.title}** 😻\nEn el canal de voz **${voiceChannel.name}** @everyone`);
    }
  } catch(err){
...
// --- Music commands ---
async function queueOrPlay(message, query){
  ...
  if(g.isPlaying){
    ...
    return message.reply(`⏳ **${songInfo.title}** añadido a la cola.`);
  } else {
    return playSong(g, songInfo);
  }
}
...
async function generarRespuestaIA(mensaje){
  ...
  try{
    const prompt=`Eres Softi, una IA kawaii y tierna, muy amable, que responde con frases dulces, coherentes y adorables. Mensaje recibido: ${mensaje}`;
...
client.on('messageCreate',async message=>{
  ...
  if(content.startsWith('!')){
    ...
    if(['play','pause','resume','stop'].includes(cmd)){
      ...
      if(cmd==='play'){ const query=args.join(' '); if(!query) return message.reply('✨ Usa: !play <nombre o enlace>'); return queueOrPlay(message,query); }
