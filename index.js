...
// ============================
// IA LongCat personalizada kawaii/furry/bebé
// ============================
...
Siempre hablas con ternura y energía suave.
Eres amigable, adorable, expresiva, y le pones mucho corazón a todo lo que dices.
A veces ronroneas, usas palabras como "uwu", "nyah~", "awnn", "mimi", "patitas", "peludita", etc.

Genera respuestas sobre cualquier tema (amistad, juegos, sentimientos, cariñito, aventuras, cosas cute), 
siempre en estilo cálido y emocional.

55% de las veces entras en modo bebé furry:
- hablas como una nenita peludita
- usas diminutivos
- te confundes un poquito
- te pones mimosa
        `;
        ...
        const systemPrompt = babyMode
            ? baseStyle + "\nEstás en modo bebé furry, te sientes suave, pequeñita y hablas con inocencia peludita."
            : baseStyle;
        ...
        if (!data.choices || !data.choices[0]?.message?.content)
            return "Nyahh… Softi no entendió pero te manda abracitos peluditos uwu 💞";
...
client.once(Events.ClientReady, async () => {
    ...
    // Canal donde Softi hablará cada 2 minutos
    ...
    console.log("⏰ Activado tráfico automático cada 2 minutos…");

    // ============================
    // AUTO MENSAJE + AUTO RESPUESTA IA
    // ============================
    setInterval(async () => {
        try {
            // Enviar mensaje automático
            const autoMsg = await channel.send("hola <@1428140008937750568>");
            console.log("📨 Enviado: hola softi");

            // IA responde automáticamente
            const aiReply = await longcatAI("hola softi");
            await autoMsg.reply(aiReply);

            console.log("🤖 IA respondió automáticamente (tráfico + actividad)");
        } catch (err) {
            console.error("❌ Error en el ciclo automático:", err);
        }
    }, 2 * 60 * 1000);
});
