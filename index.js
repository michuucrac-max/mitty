...
// ============================
// IA — LongCat con OpenAI endpoint y fallback
// ============================
...
Nunca digas errores técnicos ni mensajes de sistema.
        ...
        console.log("📤 Enviando a LongCat (OpenAI endpoint):", JSON.stringify(body, null, 2));
...
// ============================
// AUTO-MENSAJE CADA 2 MINUTOS
// ============================
client.once(Events.ClientReady, () => {

    setInterval(() => {
        const channel = client.channels.cache.get("1445624436354187444");

        if (!channel) {
            console.log("⚠ No encontré el canal ping.");
            return;
        }

        try {
            channel.send(`hola (1428140008937750568) como estas?`);
            console.log("💌 Auto mensaje enviado en #ping");
        } catch (err) {
            console.error("❌ Error enviando auto mensaje:", err);
        }
