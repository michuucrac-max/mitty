/**
 * index.js - Softti Tales (unificado)
 * - Automod, autoupdate.js (si existe), server.js (keepalive)
 * - Música con play-dl + @discordjs/voice
 * - IA para respuestas y descripción de imágenes (OpenAI API)
 * - Comandos desde cmd.json, estados desde estados.json
 * 
 * Nota: usa environment variables para TOKEN, CLIENT_ID, OPENAI_API_KEY, MUSIC_TEXT_CHANNEL_ID, MUSIC_VOICE_CHANNEL_ID
 */
...
// --- Intentar autoupdate y server.js ---
...
// --- Music structures ---
const guildPlayers = new Map(); // guildId -> { player, connection, currentSong }
...
client.on('messageCreate', async message => {
  try {
    ...
    const isMusicChannel = textChannel && message.channel.id === textChannel.id;
    ...
    if (shouldUseIA && (!textChannel || message.channel.id !== textChannel.id)) {
