...
const cmd = safeReadJSON('comandos.json', []);
...
if (statusFile) {
  if (Array.isArray(statusFile.estados)) {
    statusConfig.estados = statusFile.estados
      .map(e => ({ tipo: e.tipo || 'PLAYING', mensaje: e.mensaje || '' }))
      .filter(e => e.mensaje);
...
const vocabulario = vocabularioFile
  ? { respuestas: vocabularioFile.respuestas || [], detectar: vocabularioFile.detectar || [] }
  : { respuestas: [], detectar: [] };
...
// Filtrar mensajes por seguridad
  ...
  if (data.msgs.length > (security.antispam.maxMensajes || 5)) {
    ...
    try {
      if (message.member?.timeout) {
        await message.member.timeout((security.antispam.timeoutSegundos || 3600) * 1000, 'Spam detectado');
      }
...
// Responder con OpenAI kawaii/furry/uwu
...
client.once('ready', () => {
  ...
  cambiarEstado();
  setInterval(cambiarEstado, Math.max(1, parseInt(statusConfig.intervaloMinutos || 5)) * 60 * 1000);
});
