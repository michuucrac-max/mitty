/**
 * index.js - Softti Tales (Definitivo)
 * Música con cola, IA, AutoMod, imágenes, comandos y estados.
 */
...
function getOrCreateGuildPlayer(guild){
  if(guildPlayers.has(guild.id)) return guildPlayers.get(guild.id);

  const player = createAudioPlayer({behaviors:{noSubscriber:NoSubscriberBehavior.Pause}});
  const entry = {player,connection:null,currentSong:null,queue:[],isPlaying:false};
  guildPlayers.set(guild.id,entry);

  player.on(AudioPlayerStatus.Idle,async ()=>{
    const g = guildPlayers.get(guild.id);
    if(g.queue.length>0){
      const nextSong = g.queue.shift();
      await playSong(g,nextSong);
    } else {
      g.isPlaying=false;
      g.currentSong=null;
    }
  ...
  player.on('error',err=>console.error(`[DEBUG][guild ${guild.id}] PLAYER ERROR:`,err));
  return entry;
}

async function playSong(g,songInfo){
  try{
    if(!g.connection){
      const guild = await client.guilds.fetch(g.player.guildId);
      const voiceChannel = getVoiceChannel(guild);
      if(!voiceChannel) return console.error('No hay canal de voz.');
    ...
    if(textChannel && voiceChannel){
      textChannel.send(`🎵 Ahora reproduciendo **${songInfo.title}** 😻\nEn el canal de voz **${voiceChannel.name}** @everyone`);
    }
  }catch(err){ console.error('Error en playSong:',err); }
}

// --- Play command ---
async function playMusic(message,query){
  const guild = message.guild;
  if(!guild) return message.reply('💔 Solo funciona en servidores.');

  const textChannel = getTextChannel(guild);
  if(!textChannel) return message.reply('❌ No encontré canal de texto de música.');

  const voiceChannel = getVoiceChannel(guild);
  if(!voiceChannel) return message.reply('❌ No encontré canal de voz de música.');

  const g = getOrCreateGuildPlayer(guild);
  let songInfo;
  try{
    if(play.yt_validate(query)==='video'){
      ...
      if(!search?.length) return message.reply('😿 No encontré resultados.');
      songInfo = search[0];
    }

    g.queue.push(songInfo);
    if(!g.isPlaying) await playSong(g,g.queue.shift());
    else message.reply(`🎵 **${songInfo.title}** agregada a la cola! (${g.queue.length} en cola)`);

  }catch(err){ console.error(err); return message.reply('❌ Error al reproducir la canción.'); }
}
...
async function generarRespuestaIA(mensaje){
  ...
  try{
    const prompt=`Eres Softi, una IA kawaii y tierna, muy amable, que responde con frases dulces, coherentes y adorables. Mensaje: ${mensaje}`;
