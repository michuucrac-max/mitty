...
client.on("messageCreate", async (msg) => {
    ...
    // ★ MENSAJE ESPECIAL (solo agregado, nada más cambiado)
    ...
    if (!client._softiFirstTalk.has(msg.author.id)) {
        ...
        try {
            await msg.reply(
                `Holiii 💞 gracias por hablarme por primera vez ✨\nSi quieres invitarme a tu server, usa este enlace que nunca falla:\nhttps://discord.com/oauth2/authorize?client_id=${CLIENT_ID}&permissions=0&integration_type=0&scope=bot`
            );
        } catch {}
    }
