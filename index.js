...
client.on('messageCreate', async message => {
  try {
    ...
    // 3) Comandos globales con placeholders
    ...
    if (cmdName) {
      ...
      if (response) {
        ...
        logInfo('Command', `Comando ejecutado: /${cmdName} -> ${replyText}`);
        return;
      } else {
    ...
    // 5) Detectar imágenes y responder
...
client.on('ready', () => {
  ...
  cambiarEstado();
  setInterval(cambiarEstado, 20000); // cambia cada 20 segundos
});
