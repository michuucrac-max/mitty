/**
 * index.js - Softti Tales (unificado y DM-safe)
 * - Funciona en servidores y DMs
 * - Música con play-dl + @discordjs/voice
 * - IA OpenAI para respuestas y descripción de imágenes
 * - Automod y comandos personalizados
 */
...
// --- Optional autoupdate and keepalive ---
...
function getOrCreateGuildPlayer(guildId) {
  ...
  player.on(AudioPlayerStatus.Playing, () => console.log(`[guild ${guildId}] player PLAYING`));
  player.on(AudioPlayerStatus.Idle, () => console.log(`[guild ${guildId}] player IDLE`));
  return entry;
}
...
async function playMusic(message, query) {
  ...
  if (!textChannel) return message.reply(`❌ No encontré el canal de texto ${MUSIC_TEXT_CHANNEL_NAME}`);
  ...
  if (!voiceChannel) return message.reply(`❌ No encontré el canal de voz ${MUSIC_VOICE_CHANNEL_NAME}`);
  ...
  } catch (err) { console.error(`[guild ${guild.id}] Error playMusic:`, err); await message.reply('❌ Error al reproducir la canción'); }
}

async function pauseGuildMusic(message) { const g = guildPlayers.get(message.guild.id); if (!g) return message.reply('❌ No hay música en reproducción.'); g.player.pause(); return message.reply('⏸️ Música pausada.'); }
async function resumeGuildMusic(message) { const g = guildPlayers.get(message.guild.id); if (!g) return message.reply('❌ No hay música pausada.'); g.player.unpause(); return message.reply('▶️ Música reanudada.'); }
async function stopGuildMusic(message) { const conn = getVoiceConnection(message.guild.id); if (!conn) return message.reply('⚠️ No estoy en ningún canal de voz.'); const g = guildPlayers.get(message.guild.id); if (g && g.player) g.player.stop(); try { conn.destroy(); } catch {} guildPlayers.delete(message.guild.id); return message.reply('🛑 Reproducción detenida y desconectado.'); }
...
async function generarRespuestaIA(mensaje) {
  ...
  } catch (e) { console.error('Error OpenAI:', e); return 'Ups... no pude generar respuesta >.<'; }
}
...
async function analizarImagenYResponder(attachment, message) {
  ...
  try {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        model: 'gpt-4.1-mini',
        input: [{ role: 'user', content: [{ type: 'input_text', text: prompt }, { type: 'input_image', image_url: attachment.url }] }],
  ...
  } catch (e) { console.error('Error analizarImagenYResponder:', e); await message.reply('Ups... no pude analizar la imagen >.<'); }
}
