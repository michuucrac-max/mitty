...
client.on("messageCreate", async (msg) => {
    ...
    if (msg.author.id === OWNER_ID) {
        finalResp = `Hola papi 💗 ${aiResponse}`;
    }
...
// ============================
// INFO DE SERVIDORES Y ESTADÍSTICAS
// ============================
async function infoSofti() {
    try {
        ...
        client.guilds.cache.forEach(g => g.members.cache.forEach(m => { if (!m.user.bot) usuarios.add(m.user.username); }));
...
client.once(Events.ClientReady, () => setTimeout(infoSofti, 5000));
...
async function enviarEstadisticasCompletas() {
    try {
        ...
        if (!channel) return console.log("⚠ No pude enviar estadísticas, canal no encontrado");

        let setUsuarios = new Set();
        client.guilds.cache.forEach(g => g.members.cache.forEach(m => { if (!m.user.bot) setUsuarios.add(m.user); }));
...
client.once("ready", () => setTimeout(enviarEstadisticasCompletas, 3000));
