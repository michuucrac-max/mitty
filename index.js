...
// ============================
// Cargar comandos desde cmd.json (archivo en raíz, NO carpetas)
// ============================
...
async function longcatAI(message) {
    try {
        const systemPrompt = `
Eres Softi, una IA kawaii, furry y adorable.
Hablas suave, tierno, estilo uwu, pero SIN modo bebé.
No menciones errores internos.
`;
        ...
        return "Ay… algo falló, vuelve a intentarlo uwu 💗";
    }
}
...
client.on("messageCreate", async (msg) => {
    ...
    if (!client._softiFirstTalk) client._softiFirstTalk = new Set();
    if (!client._softiFirstTalk.has(msg.author.id)) {
        client._softiFirstTalk.add(msg.author.id);
        try {
            await msg.reply(
                `Holiii 💞 gracias por hablar conmigo ✨\nSi quieres invitarme a tu server, usa este enlace:\nhttps://discord.com/oauth2/authorize?client_id=${CLIENT_ID}&permissions=0&integration_type=0&scope=bot`
            );
        } catch {}
    }
...
// ============================
// SERVIDOR PARA RENDER (SIN CARPETAS)
// ============================
