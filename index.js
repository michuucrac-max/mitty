...
try {
  ...
  console.warn("⚠️ No se pudo cargar estados.json");
}
...
try {
  ...
  console.warn("⚠️ No se pudo cargar cmd.json");
}
...
client.on("messageCreate", async message => {
  ...
  if (isOwner && message.channel.name === "softitales-config") {
    ...
    // Info de Softi al nombrar en canal owner
    if (args[0].toLowerCase().includes("softi")) {
      const embed = new EmbedBuilder()
        .setTitle("🌸 Softi Info 🌸")
        .setColor(0xffb6c1)
        .setDescription("Aquí puedes editar configuración del servidor y ver los comandos disponibles para este servidor.");
      cmds.forEach(c => embed.addFields({ name: c.name, value: c.description, inline: true }));
      return message.reply({ embeds: [embed] });
    }
  }

  // /softihelp para todos
...
client.on("guildBoost", guild => {
  const config = loadServerConfig(guild.id);
  if (config.boostChannel) {
    const ch = guild.channels.cache.get(config.boostChannel);
    if (ch) ch.send(config.boostMessage);
  }
});
