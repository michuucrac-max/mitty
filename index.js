...
// ============================
// AutoMod (ÚNICO CAMBIO PEDIDO)
// ============================
import { automodCheck } from "./softitales-config.js";
...
// ============================
// AutoMod ejecutándose
// ============================
client.on(Events.MessageCreate, message => {
    if (message.author.bot) return;

    const result = automodCheck(message);

    if (result.action === "delete") {
        message.delete().catch(() => {});
        if (result.reply) {
            message.channel.send(result.reply).catch(() => {});
        }
    }
});
