...
function setRandomPresence() {
  ...
  client.user.setPresence({
    ...
    status: 'online',
  }).catch(err => console.error('❌ Error seteando status:', err));
}
...
client.on('messageCreate', async (message) => {
  ...
  if (content.startsWith('/')) {
    ...
    // Configuración del servidor solo si no es DM
    if (message.guild && userId === OWNER_ID) {
  ...
  // --- Preparar contexto IA kawaii/furry ---
  const context = [
    {
      role: 'system',
      content: `
Eres un asistente kawaii/furry/uwu/kwai que responde en español de manera adorable y tierna.
Usa expresiones como 'uwu', 'nya~', 'owo', emojis, y habla con mucha energía y ternura.
Haz comentarios lindos y amigables, pero nunca ofensivos.
`
    },
  ...
  if (memoria[userId]) {
    context.push({ role: 'system', content: `El usuario ha guardado esto en su memoria: ${memoria[userId].join(', ')}` });
  }
...
// --------------------------
client.on('error', (err) => {
  console.error('❌ Error en el cliente Discord.js:', err);
});

client.on('shardError', (err) => {
  console.error('❌ Error en un shard:', err);
});
