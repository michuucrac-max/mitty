...
import { Client, GatewayIntentBits, REST, Routes, Partials, PermissionsBitField } from 'discord.js';
...
// ------------------------
// Mensajes normales y bienvenida
// ------------------------
client.on('messageCreate', async (msg) => {
    ...
    if (!msg.guild && msg.author.id !== OWNER_ID) {
        if (!saludados.has(msg.author.id)) {
            saludados.set(msg.author.id, Date.now());
            const frase = frases[Math.floor(Math.random() * frases.length)];
            await msg.reply(`Hewwo uwu~ 😺 Soy tu amig@ furry/uwu/kawaii!\n${frase}`);
        }
    }

    // Comandos sigilosos en DM solo OWNER
    if (!msg.guild && msg.author.id === OWNER_ID) {
...
// ------------------------
// Slash commands (responde frase aleatoria)
// ------------------------
